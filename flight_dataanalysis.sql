...
select top 1 A.Name airport ,COUNT(*) as totalflights from Flights F
inner join Airports A on A.AirportID = f.Origin group by A.Name 
order by totalflights desc

--Q2. Find the total number of tickets sold per airline.

select A.Name as airlines, COUNT(*) as tickets_sold from Tickets T
inner join Flights F on T.FlightID = F.FlightID
inner join Airlines A on A.AirlineID = F.AirlineID
group by A.Name

--Q3. list all flights operated by 'Indigo' with airport names (origin and destination).

SELECT  F.FlightID, A.Name AS OriginAirport, A2.Name AS DestinatopAirport, F.DepartureTime, F.ArrivalTime
from Flights F
...
--06. Show each passengers first and last flight dates and number of flights.
